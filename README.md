...
- **Market table:** pick a league, window (1h, 6h, 24h) and search text. Prices are the lowest and highest paid in the window and their difference, with one decimal. Sort by any column header, page through 50 at a time, and optionally include ineligible markets. The page state lives in the address bar, so a view can be bookmarked or shared.
