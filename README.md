...
- **Gold:** *Gold/flip* is the gold needed to buy one unit at Low and sell it at High, and *Per 1k gold* is what that flip earns per 1,000 gold. An order costs the wanted item's fee per unit wanted.
- **Views:** *Chaos* and *Divine* list markets quoted in that currency. *Chaos → Divine* lists items traded in both. It assumes you buy at the Chaos market's Low and sell in bulk at the Divine market's High, with Divines valued at the window's Chaos/Divine rate. It shows margin, margin %, gold for both orders and a score = Margin % × Chaos/h (the slower market) × Per 1k gold. Its detail panel shows both markets' histories, so you can tell a lasting gap from a price move within an hour.
- **Market table:** pick a league, a view, a window (1h finds opportunities; 24h is context) and search text. In the 1h window, *Held* shows how many of the 6 previous hours had at least half the newest hour's margin, and *moving* marks a price that drifted by more than the margin, meaning a price move rather than a lasting gap. Moving rows are not ranked, and the score counts only the held share. Prices are the lowest and highest paid in the window and their difference, with one decimal. Sort by any column header, page through 50 at a time, and optionally include ineligible markets. The page state lives in the address bar, so a view can be bookmarked or shared.
- **Market detail:** click an item to see its summary, a chart of each hour's lowest and highest traded price (with the band between them) and Chaos traded, and a strip showing each hour's status. Hours without trades break the lines; hours without data are hatched. History covers 24h, 7d or 30d.
...
| `GET /api/markets?league=<name>` | Markets of a league with their metrics, quoted in `quote=chaos\|divine` (chaos). `window=1h\|24h` (1h), `scope=eligible\|all` (eligible), `sort=rank\|score\|gold\|per_gold\|held\|turnover\|units\|persistence\|volatility\|rate\|low\|high\|range\|name` (rank; `range` is high minus low in the quote), `order=asc\|desc`, `q=<text>` (name or path), `limit` 1–100 (50), `offset` 0–10,000. `meta.total` counts all matches. |
| `GET /api/markets/<id>/history?league=<name>` | Takes the same `quote`. One point per hour, ending at the newest parsed hour, plus a summary over the window. `window=24h\|7d\|30d` (24h). Every hour has a `status`: `missing`, `exchange-down`, `league-absent` (all three unknown), `inactive`, `listed` or `traded`. Gaps are never left out or zero-filled. |
| `GET /api/flips?league=<name>` | Chaos → Divine flips for items with both a Chaos and a Divine market: `buy` (Chaos Low), `sell` (Divine High), `sell_chaos`, `margin_chaos`, `margin_share`, `turnover_per_hour` (slower market, in Chaos), `gold_per_flip`, `chaos_per_1k_gold`, `score` and `rank`, with `meta.divine_rate`. Same `window`, `scope`, `order`, `q`, `limit` and `offset` as markets; `sort=rank\|score\|margin\|margin_pct\|per_gold\|gold\|turnover\|buy\|sell\|held\|name`. Built from the two metric snapshots when requested. |
...
- `items.gold_fee` is the currency exchange's gold fee per unit wanted, from [`data/gold-fees.json`](./data/gold-fees.json) (the game's `CurrencyExchange` table via [repoe-fork/dat-export](https://github.com/repoe-fork/dat-export)). Apply it with `npm run gold-fees`, and refresh it each league or patch with `npm run gold-fees -- --download`. Review the diff, commit it, and rerun `npm run metrics`. See [Market metrics: Gold](./specs/market-metrics.md#gold).
